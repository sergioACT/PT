...
const PanicButton = props => {
  ...
  useEffect(() => {
    ...
    if (hasEmergency) {
      timeout = setInterval(() => {
        setBackgroundColor(backgroundColor => (backgroundColor == 'purpleColor' ? 'redColor' : 'purpleColor'));
      }, 500);
    } else {
  ...
  const rStyle = useAnimatedStyle(() => {
    const backgroundColorS = interpolateColor(progress.value, [0, 1], [Colors.redColor.background, Colors.purpleColor.background]);
